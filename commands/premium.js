...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('premium')
    .setDescription('Premium bir hesap verir.')
    .addStringOption(opt =>
      opt.setName('service')
         .setDescription('Hizmet adı (ör: valorant, neon)')
         .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    const allowed = config.allowedChannels["premium"];
    ...
    const cooldownSeconds = config.cooldowns["premium"];
    ...
    const dosya = path.join(__dirname, `../premium/${service}.txt`);
    ...
    const kanalEmbed = new EmbedBuilder()
      ...
      .setDescription(`🎁 **Kalan Stok:** ${satirlar.length}\n\n✨ **Premium hesap DM’den gönderildi!**`)
      .setColor(0xffd700)
      .setFooter({
        text: `${interaction.user.tag} • ${new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}`,
        iconURL: interaction.user.displayAvatarURL()
